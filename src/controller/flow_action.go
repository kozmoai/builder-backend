...
import (
	...
	"github.com/kozmoai/builder-backend/src/utils/kozmoresourcemanagersdk"
...
func (controller *Controller) CreateFlowAction(c *gin.Context) {
	...
	if createFlowActionRequest.IsRemoteVirtualAction() {
		// the AI_Agent need fetch resource info from resource manager, but kozmo drive does not need that
		if createFlowActionRequest.NeedFetchResourceInfoFromSourceManager() {
			api, errInNewAPI := kozmoresourcemanagersdk.NewkozmoResourceManagerRestAPI()
			if errInNewAPI != nil {
