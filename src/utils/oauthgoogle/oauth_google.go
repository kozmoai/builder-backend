...
func ExchangeOAuthToken(code string) (*ExchangeTokenResponse, error) {
	...
	googleOAuthClientID := conf.GetKozmoGoogleSheetsClientID()
	googleOAuthClientSecret := conf.GetKozmoGoogleSheetsClientSecret()
	googleOAuthRedirectURI := conf.GetKozmoGoogleSheetsRedirectURI()
	client := resty.New()
	// request
	resp, _ := client.R().
