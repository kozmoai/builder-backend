...
const (
	FILE_TINY_URL_PREFIX_TEST       = "https://cloud-api-test.kozmoai.com/drive/f/%s"
	FILE_TINY_URL_PREFIX_BETA       = "https://cloud-api-beta.kozmoai.com/drive/f/%s"
	FILE_TINY_URL_PREFIX_PRODUCTION = "https://cloud-api.kozmoai.com/drive/f/%s"
)

// the function response are fileID => fileTinyURL lookup map.
func (r *KozmoDriveRestAPI) generateDriveTinyURLs(fileIDs []string, expirationType string, expiry string, hotlinkProtection bool) (map[string]string, error) {
	...
	// ```
	client := resty.New()
	uri := r.Config.GetKozmoDriveAPIForSDK() + fmt.Sprintf(DRIVE_API_GENERATE_TINY_URL_BATCH, idconvertor.ConvertIntToString(r.TeamID))
