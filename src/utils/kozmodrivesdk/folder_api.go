...
func (r *KozmoDriveRestAPI) getFolderIDByPath(path string) (string, error) {
	...
	// ```
	client := resty.New()
	uri := r.Config.GetKozmoDriveAPIForSDK() + fmt.Sprintf(DRIVE_API_GET_FOLDER_ID_BY_PATH, idconvertor.ConvertIntToString(r.TeamID), params)
	resp, errInGet := client.R().
		SetHeader("Action-Token", actionToken).
		Get(uri)
	if r.Debug {
		log.Printf("[DUMP] KozmoDriveSDK.GetFolderIDByPath() uri: %+v \n", uri)
		log.Printf("[DUMP] KozmoDriveSDK.GetFolderIDByPath() response: %+v, err: %+v \n", resp, errInGet)
		log.Printf("[DUMP] KozmoDriveSDK.GetFolderIDByPath() resp.StatusCode(): %+v \n", resp.StatusCode())
	}
	if errInGet != nil {
		return "", errInGet
	}
