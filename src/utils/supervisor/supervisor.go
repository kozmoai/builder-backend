...
func (supervisor *Supervisor) ValidateUserAccount(token string) (bool, error) {
	...
	if resp.StatusCode() != http.StatusOK {
		if err != nil {
			return false, errors.New("request kozmo supervisor failed.")
		}
		return false, nil
	}
	return true, nil
...
func (supervisor *Supervisor) GetTeamPermissions(teamID int) (string, error) {
	...
	if resp.StatusCode() != http.StatusOK {
		if err != nil {
			return "", errors.New("request kozmo supervisor failed.")
		}
...
func (supervisor *Supervisor) CanAccess(token string, teamID int, unitType int, unitID int, attributeID int) (bool, error) {
	...
	if resp.StatusCode() != http.StatusOK {
		if err != nil {
			return false, errors.New("request kozmo supervisor failed: " + err.Error())
		}
		return false, nil
	}
	return true, nil
