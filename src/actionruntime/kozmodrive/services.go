...
import (
	...
	kozmodrivesdk "github.com/kozmoai/builder-backend/src/utils/kozmodrivesdk"
)
...
type KozmoDriveConnector struct {
	Action KozmoDriveTemplate
}

// AI Agent have no validate resource options method
func (r *KozmoDriveConnector) ValidateResourceOptions(resourceOptions map[string]interface{}) (common.ValidateResult, error) {
	return common.ValidateResult{Valid: true}, nil
}

func (r *KozmoDriveConnector) ValidateActionTemplate(actionOptions map[string]interface{}) (common.ValidateResult, error) {
...
func (r *KozmoDriveConnector) TestConnection(resourceOptions map[string]interface{}) (common.ConnectionResult, error) {
	return common.ConnectionResult{Success: false}, errors.New("unsupported type: AI Agent")
}

// AI Agent have no meta info
func (r *KozmoDriveConnector) GetMetaInfo(resourceOptions map[string]interface{}) (common.MetaInfoResult, error) {
	return common.MetaInfoResult{Success: false}, errors.New("unsupported type: AI Agent")
}

func (r *KozmoDriveConnector) Run(resourceOptions map[string]interface{}, actionOptions map[string]interface{}, rawActionOptions map[string]interface{}) (common.RuntimeResult, error) {
	...
	fmt.Printf("[DUMP] kozmodrive.Run() actionOptions: %+v\n", actionOptions)
	...
	// init
	driveAPI := kozmodrivesdk.NewKozmoDriveRestAPI(teamID, userID, instanceType, instanceID)
	driveAPI.OpenDebug()

	// operation filter for kozmo drive sdk function call
	switch operation {
	case kozmodrivesdk.DRIVE_API_ACTION_LIST_FILES:
	...
	case kozmodrivesdk.DRIVE_API_ACTION_GET_UPLOAD_ADDRESS:
		...
		res.Rows = append(res.Rows, ret)
	case kozmodrivesdk.DRIVE_API_ACTION_UPDATE_FILE_STATUS:
		...
		res.Rows = append(res.Rows, ret)
	case kozmodrivesdk.DRIVE_API_ACTION_GET_MULTIPLE_UPLOAD_ADDRESS:
		...
		res.Rows = ret
	case kozmodrivesdk.DRIVE_API_ACTION_GET_DOWNLOAD_ADDRESS:
		...
		res.Rows = append(res.Rows, ret)
	case kozmodrivesdk.DRIVE_API_ACTION_GET_MULTIPLE_DOWNLOAD_ADDRESS:
	...
	case kozmodrivesdk.DRIVE_API_ACTION_DELETE_FILE:
		...
		res.Rows = append(res.Rows, ret)
	case kozmodrivesdk.DRIVE_API_ACTION_DELETE_MULTIPLE_FILE:
		...
		res.Rows = append(res.Rows, ret)
	case kozmodrivesdk.DRIVE_API_ACTION_RENAME_FILE:
		// / get mutiple upload address require field: teamID int, fileID string
